...
struct CoherentDedisperserConfig{
    ...
    //thrust::device_vector<cufftComplex> _d_ism_responses; // flattened array of ISM responses following the order of DMs, channels and sub-channels
    std::vector<thrust::device_vector<cufftComplex>> _d_ism_responses;

    cufftHandle _fft_plan;
    // cufftHandle _i_fft_plan;
};
void get_dm_responses(CoherentDedisperserConfig& config, float dm_prefactor, thrust::device_vector<cufftComplex>& responses);
...
public:
    static void createConfig(CoherentDedisperserConfig& config, std::size_t num_fine_chans, std::size_t num_coarse_chans, std::size_t num_pols, std::size_t num_antennas, float tsamp, float low_freq, float bw, std::vector<float> dms);
    CoherentDedisperser(PipelineConfig const& pipeline_config, CoherentDedisperserConfig& config) : pipeline_config(pipeline_config), config(config) { }
    ~CoherentDedisperser();
    void dedisperse(thrust::device_vector<char2> const& d_tpa_voltages_in, thrust::device_vector<char2>& d_ftpa_voltages_out, std::size_t out_offset, int dm_idx);
...
private:
    ...
    void multiply_by_chirp(thrust::device_vector<cufftComplex> const& d_fpa_spectra_in, 
