#ifndef SKYWEAVER_SIGPROCFILEWRITER_HPP
#define SKYWEAVER_SIGPROCFILEWRITER_HPP
...
class SigprocFileWriter
{
  ...
  public:
     ...
     *
     * @param      block  A psrdada_cpp::RawBytes object containing metadata in
     * sigproc format
     */
     ...
     *
     * @param      block  A psrdada_cpp::RawBytes object containing data in
     * sigproc format
     */
     ...
     *
     * @detail     Calling this will change the writer state to enabled, and if
     * necessary set flags to indicate that a new file stream is required.
     * Changes will only come into effect on the next call to the operator()
     * method.
     *
     * @note       Has no effect if the writer is already enabled. To trigger a
     * new stream the writer must first be disabled then enabled.
     */
    void enable();
     ...
     *
     * @detail     Calling this will change the writer state to disabled. If
     * there is an active file stream the stream will be flushed to disk and
     * closed. Changes will only come into effect on the next call to the
     * operator() method. If and enable call comes before the next call to the
     * operator method the system will simply generate a new stream when the
     * operator() method is called (unless another disable comes in before
     * that... and so on).
     *
     ...
     *
     * @return     A reference to the FilHead object storing the observation
     * metadata
     */
    FilHead& header();
...
#endif // SKYWEAVER_SIGPROCFILEWRITER_HPP
