...
class SkyCleaver
{
  ...
  private:   
    ...
    std::map<DMNumberType, std::map<BeamNumberType, std::unique_ptr<MultiFileWriter<OutputVectorType>>>> _beam_writers;
    std::map<DMNumberType, std::map<BeamNumberType, std::shared_ptr<OutputVectorType>>> _beam_data;
