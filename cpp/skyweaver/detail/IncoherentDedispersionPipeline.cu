...
template <typename InputType, typename OutputType, typename Handler>
void IncoherentDedispersionPipeline<InputType, OutputType, Handler>::agg_buffer_callback(typename InputVectorType::VectorType const& buffer, std::size_t block_idx)
{
    BOOST_LOG_TRIVIAL(debug) << "Agg buffer callback called for block_idx = " << block_idx;
