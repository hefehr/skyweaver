...
MultiFileWriter<VectorType>::MultiFileWriter(PipelineConfig const& config, std::string tag)
...
template <typename VectorType>
FileStream& MultiFileWriter<VectorType>::create_stream(VectorType const& stream_data, std::size_t stream_idx)
{   
    BOOST_LOG_TRIVIAL(info) << "Creating stream based on stream prototype: " << stream_data.describe();
    // Here we round the file size to a multiple of the stream prototype
    std::size_t filesize = std::max(1ul, _config.max_output_filesize() / stream_data.size() / sizeof(typename VectorType::value_type)) * stream_data.size();
    BOOST_LOG_TRIVIAL(debug)  << "Maximum allowed file size = " << filesize << " bytes (+header)";
    _file_streams[stream_idx] = std::make_unique<FileStream>(
        ...
        [&, this, stream_data, stream_idx, filesize](
            ...
            std::memcpy(temp_header, default_dada_header.c_str(), default_dada_header.size());
            ...
            header_writer.set<long double>("UTC_START", _header.utc_start + stream_data.utc_offset());
...
template <typename VectorType>
std::string MultiFileWriter<VectorType>::get_basefilename(VectorType const& stream_data, std::size_t stream_idx)
{
    // Output file format
    ...
    base_filename << get_formatted_time(_header.utc_start) << "_stream" 
                  << stream_idx << "_" << std::fixed
                  << std::setprecision(3) << std::setfill('0')
                  << std::setw(9) << stream_data.reference_dm();
    if(!_tag.empty()) {
...
template <typename VectorType>
std::string MultiFileWriter<VectorType>::get_extension(VectorType const& stream_data)
{   
                                             ...
                                             std::size_t stream_idx)
{
    ...
    {
        thrust::host_vector<typename VectorType::value_type> stream_data_h = stream_data.vector();
        _file_streams.at(stream_idx)->write(
            reinterpret_cast<char const*>(stream_data_h.data()),
            stream_data_h.size() * sizeof(typename VectorType::value_type));
    } else {
        _file_streams.at(stream_idx)->write(
            reinterpret_cast<char const*>(stream_data.data()),
