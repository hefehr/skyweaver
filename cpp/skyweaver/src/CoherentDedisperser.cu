...
{
    return ((1 / pow(f1 / 1000, 2.0)) - (1 / pow(f2 / 1000, 2.0))) * dm * 0.00415;
}
    ...
    unsigned int dm_idx)
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "Input TPA voltages to dedisperse, d_tpa_voltages_in.size(): " << d_tpa_voltages_in.size();
    BOOST_LOG_TRIVIAL(debug) << "Output FTPA voltages to write to, d_ftpa_voltages_out.size(): " << d_ftpa_voltages_out.size();
    ...
    std::size_t out_offset = freq_idx * config.nantennas * config.npols * (config.fft_length -config.overlap_samps);
    std::size_t discard_size = config.nantennas * config.npols * config.overlap_samps /2 ;


    BOOST_LOG_TRIVIAL(debug) << "Output offset to write from: " << out_offset;
    BOOST_LOG_TRIVIAL(debug) << "discard_size: " << discard_size;
    BOOST_LOG_TRIVIAL(debug) << "copying from input from " << discard_size << " to " << d_tpa_voltages_out_temp.size() - discard_size;
    BOOST_LOG_TRIVIAL(debug) << "copying to output from " << out_offset << " to "
    ...
    unsigned int dm_idx)
{
    ...
    kernels::dedisperse<<<gridSize, blockSize>>>(
        thrust::raw_pointer_cast(config._d_ism_responses[dm_idx].data() + response_offset),
                           ...
                           unsigned total_chans)
{
    ...
    const unsigned remainder = min(total_chans - block_start_chan_idx,
                              NCHANS_PER_BLOCK); 
    ...
    Each thread processes NCHANS_PER_BLOCK channels sequentially, for a given (iant, ipol)
    **/

    for(int block_ichan_idx = 0; block_ichan_idx < remainder; ++block_ichan_idx) {
        const int chan_idx = (block_start_chan_idx  + block_ichan_idx ) * pa_size  // get to the correct chan_idx
...
struct DMResponse {
    ...
    DMResponse(int num_coarse_chans,
               ...
               double fine_chan_bw,
               double dm_prefactor)
        : num_coarse_chans(num_coarse_chans), num_fine_chans(num_fine_chans), low_freq(low_freq), 
          coarse_chan_bw(coarse_chan_bw), fine_chan_bw(fine_chan_bw),
          dm_prefactor(dm_prefactor)
    {
    }
    ...
    __device__ inline cufftComplex operator()(int tid) const
    {
        ...
        double nu_0 = low_freq + chan * coarse_chan_bw  - 0.5f * coarse_chan_bw; // + fine_chan * fine_chan_bw;

        double nu =  fine_chan * fine_chan_bw; // fine_chan_freq

        double phase_prefactor = nu * nu * dm_prefactor;
        double phase = phase_prefactor / ((nu_0 + nu) * nu_0 * nu_0); // precalculate
