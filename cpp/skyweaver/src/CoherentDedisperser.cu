...
#include "cufft.h"
namespace skyweaver { 
        void CoherentDedisperser::createConfig(CoherentDedisperserConfig& config, std::size_t num_fine_chans, std::size_t num_coarse_chans, std::size_t num_pols, 
                                                                                std::size_t num_antennas, float tsamp, float low_freq, 
                                                                                float bw,  std::vector<float> dms){
            ...
            thrust::transform(config._d_dms.begin(), 
                            config._d_dms.end(), 
                            config._d_dm_prefactor.begin(), [=] __device__ (float dm){
                                        return -1.0f * TWO_PI * DM_CONST * dm;
                                    });
            ...
            //config._d_ism_responses.reserve(config.num_coarse_chans * config.num_fine_chans * config._d_dms.size()); 
            for(int idx = 0; idx < config._d_dm_prefactor.size(); idx++){
                get_dm_responses(config, config._d_dm_prefactor[idx], config._d_ism_responses[idx]);
            }

            //data is FTPA order, we will loop over F, so we are left with TPA order. Let's fuse PA to X, so TX order. 
            ...
            if(cufftPlanMany(&config._fft_plan, 1, n,
    ...
    **/
    ...
    void CoherentDedisperser::dedisperse(thrust::device_vector<char2> const& d_tpa_voltages_in, thrust::device_vector<char2>& d_ftpa_voltages_out, std::size_t out_offset, int dm_idx){
        //make them members
        ...
        multiply_by_chirp(d_fpa_spectra, d_fpa_spectra, dm_idx); // operating in place. 
        ...
        thrust::transform(d_tpa_voltages_out_temp.begin() + config.num_fine_chans / 2, 
    ...
    void CoherentDedisperser::multiply_by_chirp(thrust::device_vector<cufftComplex> const& d_fpa_spectra_in, 
        ...
        if(total_chans% NCHANS_PER_BLOCK != 0){
            throw std::runtime_error("Total chans need to be a multiple of NCHANS_PER_BLOCK");
        }

        dim3 blockSize(pipeline_config.nantennas() * pipeline_config.npol());
        dim3 gridSize(total_chans / NCHANS_PER_BLOCK);
        kernels::dedisperse<<<gridSize, blockSize>>>(thrust::raw_pointer_cast(config._d_ism_responses[dm_idx].data()), 
...
namespace skyweaver {
    namespace kernels{
        ...
        struct DMResponse {
            ...
            DMResponse(int num_coarse_chans, float low_freq, float coarse_chan_bw, float fine_chan_bw, float dmPrefix)
                : num_coarse_chans(num_coarse_chans), low_freq(low_freq), coarse_chan_bw(coarse_chan_bw), fine_chan_bw(fine_chan_bw), dmPrefix(dmPrefix),
            ...
            __device__ inline cufftComplex operator()(int tid) const {
                ...
                float edgeFreq = low_freq 
                                    + chan * coarse_chan_bw 
                                    + fine_chan * fine_chan_bw 
                                    -  fine_chan_bw * 0.5f;
                float phase = phase_prefactor / ((edgeFreq + fine_chan_bw) * edgeFreq * edgeFreq); // precalculate
                cufftComplex weight;
        ...
        void get_dm_responses(CoherentDedisperserConfig& config, float dm_prefactor, thrust::device_vector<cufftComplex>& response) {
            
            thrust::device_vector<int> indices(config.num_coarse_chans * config.num_fine_chans);
