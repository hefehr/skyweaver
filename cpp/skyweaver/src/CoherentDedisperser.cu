                                       ...
                                       std::vector<float> dms)
{
    ...
    for(int idx = 0; idx < config._d_dm_prefactor.size(); idx++) {
        BOOST_LOG_TRIVIAL(info) << "DM Prefactor: " << config._d_dm_prefactor[idx];
    }
    ...
    int dm_idx)
{
    ...
    // for(int i=0; i < 10; i++) {
    //     cufftComplex x = d_fpa_spectra[i];
    //     BOOST_LOG_TRIVIAL(debug) << "after d_fpa_spectra[" << i << "].x: " <<x.x;
    //     BOOST_LOG_TRIVIAL(debug) << "after d_fpa_spectra[" << i << "].y: " << x.y;
    // }
    ...
    std::size_t  discard_size = config.num_antennas * config.num_pols * config.overlap_samps / 2;
    BOOST_LOG_TRIVIAL(debug) << "copying from " << discard_size << " to " << d_tpa_voltages_out_temp.size() - discard_size;
    // transform: divide by d_tpa_voltages_in.size()
    thrust::transform(
        d_tpa_voltages_out_temp.begin() + discard_size,
        d_tpa_voltages_out_temp.end() - discard_size,
        d_ftpa_voltages_out.begin() + out_offset,
        [=] __device__(cufftComplex const& val) {
            char2 char2_val;
            char2_val.x = static_cast<char>(__float2int_rn(val.x / N)); // scale the data back
            char2_val.y = static_cast<char>(__float2int_rn(val.y / N));
            return char2_val;
        });
    ...
    int dm_idx)
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "config._d_ism_responses[dm_idx] size: " << config._d_ism_responses[dm_idx].size();
    BOOST_LOG_TRIVIAL(debug) << "d_fpa_spectra_in size: " << d_fpa_spectra_in.size();
    BOOST_LOG_TRIVIAL(debug) << "d_fpa_spectra_out size: " << d_fpa_spectra_out.size();


    dim3 blockSize(pipeline_config.nantennas() * pipeline_config.npol()); // 128 threads per block
...
const int start_chan_idx = blockIdx.x * NCHANS_PER_BLOCK; // 0->4096 * 128 = 0 -> 524288

const int remainder = min(total_chans - start_chan_idx, NCHANS_PER_BLOCK); // 524288 -> 0 or 128
...
const int chan_idx = ((ii + start_chan_idx)%NCHANS_PER_BLOCK) * tp_size + threadIdx.x;
...
struct DMResponse {
    ...
    __device__ inline cufftComplex operator()(int tid) const
    {
        ...
        cufftComplex weight;
        __sincosf(phase, &weight.y, &weight.x); // TO DO: test if it is not approximate
        return weight;
    }
};
