...
void DedispersionPlan::add_block(float coherent_dm,
                                 float dm_begin,
                                 float dm_end,
                                 float dm_step,
                                 int tscrunch)
...
void DedispersionPlan::add_block(std::string str)
{
    ...
    try {
        switch(parts.size()) {
        case 1: add_block(std::stof(parts[0])); break;
        case 2: add_block(std::stof(parts[0]), std::stoi(parts[1])); break;
        case 5:
        ...
        default:
            throw std::runtime_error(
                std::string("Block defition string of unexpected size: ") +
                str);
        }
    } catch(std::invalid_argument& e) {
        throw std::invalid_argument(
            std::string("Error parsing block definition string: ") + e.what());
                         ...
                         DedispersionPlanBlock const& block)
{
    stream << "Coherent DM: " << block.coherent_dm << ", "
           ...
           << "nDMs: " << block.incoherent_dms.size() << ", " << "DMs: ";
    bool first = true;
