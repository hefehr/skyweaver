...
MultiFileReader::MultiFileReader(PipelineConfig const& config): 
    _config(config), _current_file_idx(0), _current_position(0), eofFlag(false) {

    static_assert(sizeof(char2) == 2 * sizeof(char), "Size of char2 is not as expected");
    ...
    if (!files.empty()) {
        read_header();  // Read header from the first file to set _dada_header_size
    }
}
...
void MultiFileReader::open() {
    ...
    if (_dada_header_size > 0 && _current_stream.good()) {
        _current_stream.seekg(_dada_header_size, std::ios::beg);  // Skip header for subsequent files
    }
...
void MultiFileReader::seekg(std::size_t pos, std::ios_base::seekdir dir) {
    std::lock_guard<std::mutex> lock(mtx);
     std::size_t targetPos = (dir == std::ios_base::cur) ? _current_position + pos : pos;
    if (dir == std::ios_base::end && pos < 0) targetPos = this->_total_size + pos;
