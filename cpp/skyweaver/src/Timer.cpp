...
void Timer::start(const std::string& functionName) {
    if (_startTimes.find(functionName) != _startTimes.end()) {
        throw std::runtime_error("Timer already started for function: " + functionName);
    }
...
void Timer::stop(const std::string& functionName) {
    if (_startTimes.find(functionName) == _startTimes.end()) {
        throw std::runtime_error("Timer not started for function: " + functionName);
    }
...
void Timer::show_timings(const std::string& functionName) const {
    if (_timingData.find(functionName) == _timingData.end()) {
        throw std::runtime_error("No timing data for function: " + functionName);
    }

    const auto& times = _timingData.at(functionName);
    if (times.empty()) {
        throw std::runtime_error("No recorded times for function: " + functionName);
    }
    ...
    std::cout << "Statistics for function '" << functionName << "':" << std::endl;
    std::cout << "  Average time: " << std::fixed << std::setprecision(3) << average << " us" << std::endl;
