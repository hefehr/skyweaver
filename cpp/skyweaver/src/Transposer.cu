                            ...
                            int ntimestamps)
{
    ...
    __shared__ char2 transpose_buffer[SKYWEAVER_ST_MAX_ANTENNAS]
    ...
    for(int timestamp_idx = blockIdx.x; timestamp_idx < ntimestamps;
        ...
        for(int chan_idx = blockIdx.y; chan_idx < nchans;
            ...
            for(int antenna_set_idx = 0; antenna_set_idx < nantennas_sets;
                ...
                int remaining_antennas = min(
                    input_nantennas - antenna_set_idx * SKYWEAVER_ST_MAX_ANTENNAS,
                ...
                for(int antenna_idx = threadIdx.y;
                    ...
                    int input_antenna_idx =
                        antenna_set_idx * SKYWEAVER_ST_MAX_ANTENNAS + antenna_idx;
                    ...
                    for(int samppol_idx = threadIdx.x;
                        samppol_idx < (SKYWEAVER_NSAMPLES_PER_HEAP * SKYWEAVER_NPOL);
                ...
                for(int pol_idx = 0; pol_idx < SKYWEAVER_NPOL; ++pol_idx) {
                    for(int samp_idx = threadIdx.y;
                        ...
                        int output_sample_idx =
                            samp_idx + timestamp_idx * SKYWEAVER_NSAMPLES_PER_HEAP;
                           ...
                           cudaStream_t stream)
{
    ...
    BOOST_LOG_TRIVIAL(debug)
        ...
        << _config.nsamples_per_heap() * nheap_groups << ", " 
        << _config.npol() << ", "
        << _config.nantennas() << ")";
