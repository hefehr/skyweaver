...
void generate_weights_k(
    float3 const * __restrict__ delay_models,
    ...
    int ntsteps)
{
    ...
    for (int chan_idx = blockIdx.y; chan_idx < nchans; chan_idx += gridDim.y)
    {
        ...
        for (int beam_idx = blockIdx.x; beam_idx < nbeams; beam_idx += gridDim.x)
        {
            ...
            {
                float3 delay_model = delay_models[beam_idx * nantennas + antenna_idx]; // correct
                ...
                for (int time_idx = threadIdx.y; time_idx < ntsteps; time_idx+=blockDim.y)
                {
                    ...
                    compressed_weight.x = (char) __double2int_rn(weight.x * 127.0 * delay_model.x);
                    compressed_weight.y = (char) __double2int_rn(-1.0 * weight.y * 127.0 * delay_model.x);
