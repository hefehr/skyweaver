    ...
    main_options.add_options()
        ...
        ("root-prefix",
                po::value<std::string>()
                    ...
                    ->notifier(
                        [&config](std::string key) { config.root_prefix(key); }),
                "The prefix for all output files")
        ("out-prefix",
                po::value<std::string>()
                    ->default_value(config.out_prefix())
                    ->notifier(
                        [&config](std::string key) { config.out_prefix(key); }),
                "The prefix for all output files")
        ("nthreads",
                po::value<unsigned int>()
                    ->default_value(config.nthreads())
                    ->notifier(
                        [&config](unsigned int key) { config.nthreads(key); }),
        ...
        ("nchans",
                po::value<std::size_t>()
                    ...
                    ->notifier(
                        [&config](std::size_t key) { config.nchans(key); }),
                "The number of channels")
        ("nbridges",
                po::value<std::size_t>()
                    ->default_value(config.nbridges())
                    ->notifier(
                        [&config](std::size_t key) { config.nbridges(key); }),
                "The number of bridges")
        ("nbeams",
                po::value<std::size_t>()
                    ->default_value(config.nbeams())
                    ->notifier(
                        [&config](std::size_t key) { config.nbeams(key); }),
                "The number of beams")
        ("ndms", 
                po::value<std::size_t>()
                    ->default_value(config.ndms())
                    ->notifier(
                        [&config](std::size_t key) { config.ndms(key); }),
                "The number of DMs")
        ("stokes-mode",
                po::value<std::string>()
                    ->default_value(config.stokes_mode())
                    ->notifier(
                        [&config](std::string key) { config.stokes_mode(key); }),
                "The stokes mode")
        ("stream-id",
                po::value<std::size_t>()
                    ->default_value(config.stream_id())
                    ->notifier(
                        [&config](std::size_t key) { config.stream_id(key); }),
                "The stream id")
        ("max-ram-gb",
                po::value<std::size_t>()
                    ->default_value(config.max_ram_gb())
                    ->notifier(
                        [&config](std::size_t key) { config.max_ram_gb(key); }),
                "The maximum amount of RAM to use in GB")
        ("max-output-filesize",
                po::value<std::size_t>()
                    ->default_value(config.max_output_filesize())
                    ->notifier(
                        [&config](std::size_t key) { config.max_output_filesize(key); }),
        ...
        try {
            ...
            if(variable_map.count("help")) {
                std::cout << "skycleaver -- A pipeline that cleaves input TDB files, "
