...
auto pre_write_callback = [] (skyweaver::MultiFileWriterConfig const& config)
{
    if (!config.pre_write.is_enabled) return;
    std::filesystem::space_info space = std::filesystem::space(config.base_output_dir);
    size_t limit = config.pre_write.wait.min_free_space;
    if(space.available >= config.pre_write.wait.min_free_space)
        return;
    ...
    BOOST_LOG_TRIVIAL(info)
        ...
        << limit
        << " bytes in " << config.base_output_dir << ".";
    ...
    for (int i = 0; i < max_iterations; i++)
    {
        ...
        space = std::filesystem::space(config.base_output_dir);
