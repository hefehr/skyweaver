...
typedef ::testing::Types<std::vector<int8_t>,
                         std::vector<float>,
                         std::vector<char4>,
                         std::vector<float4>,
                         thrust::host_vector<int8_t>,
                         thrust::host_vector<float>,
                         thrust::host_vector<char4>,
                         thrust::host_vector<float4>,
                         thrust::device_vector<int8_t>,
                         thrust::device_vector<float>,
                         thrust::device_vector<char4>,
...
TYPED_TEST(AggregationBufferTester, PushBackTest)
{
    ...
    AggregationBuffer<T> buffer(
        std::bind(&AggregationBufferTester<ContainerType>::MockCallback,
                  this,
                  std::placeholders::_1),
        10,
        0,
...
TYPED_TEST(AggregationBufferTester, PushBackOverlappedTest)
{
    ...
    std::copy(data.begin() + 2 * 10 * (expected_callbacks - 1),
              data.begin() + 2 * 10 * (expected_callbacks) + 2,
              expected_data.begin());
