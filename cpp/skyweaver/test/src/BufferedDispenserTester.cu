...
TEST_F(BufferedDispenserTester, testBufferedDispenser)
{
    ...
    const std::size_t kernel_length_tpa =
        pipeline_config.dedisp_kernel_length_samps() * nantennas * npols;
    ...
    for(int i = 0; i < nchans; i++) {
        ...
        thrust::host_vector<char2> dispensed_voltages_1h(
            dispensed_voltages_1.size());
        thrust::copy(dispensed_voltages_1.begin(),
                     dispensed_voltages_1.end(),
                     dispensed_voltages_1h.begin());
    ...
    for(int i = 0; i < nchans; i++) {
        ...
        thrust::host_vector<char2> dispensed_voltages_2h(
            dispensed_voltages_2.size());
        thrust::copy(dispensed_voltages_2.begin(),
                     dispensed_voltages_2.end(),
                     dispensed_voltages_2h.begin());
        ...
        for(std::size_t j = 0; j < start_idx; j++) {
            ASSERT_EQ(voltages_1h[(i + 1) * size_TPA - kernel_length_tpa + j].x,
                      dispensed_voltages_2h[j].x);
            ASSERT_EQ(voltages_1h[(i + 1) * size_TPA - kernel_length_tpa + j].y,
                      dispensed_voltages_2h[j].y);
        }
    }
}
