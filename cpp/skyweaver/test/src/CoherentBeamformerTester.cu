    ...
    for (int channel_idx = 0; channel_idx < nchannels; channel_idx += fscrunch)
    {
        ...
        for (int sample_idx = 0; sample_idx < nsamples; sample_idx+=tscrunch)
        {
            for (int beam_idx = 0; beam_idx < nbeams; ++beam_idx)
            {
                ...
                int tf_size = SKYWEAVER_CB_NSAMPLES_PER_HEAP * nchannels/fscrunch;
                int btf_size = nbeams * tf_size;
                int output_sample_idx = sample_idx / tscrunch;
                int tbtf_powers_idx = (output_sample_idx / SKYWEAVER_CB_NSAMPLES_PER_HEAP * btf_size
                    + beam_idx * tf_size
                    + (output_sample_idx % SKYWEAVER_CB_NSAMPLES_PER_HEAP) * nchannels/fscrunch
                ...
                float powerf32 = ((power - (127.0f * 127.0f * ib_power))/scales[channel_idx/fscrunch]);
#else
...
TEST_F(CoherentBeamformerTester, representative_noise_test)
{
    ...
    float scale = std::pow(127.0f * input_level * std::sqrt(static_cast<float>(_config.nantennas())), 2);
    ...
    printf("Nantennas: %d, tscrunch: %d, fscrunch: %d, npol: %d, Output level: %f, Input level: %f, Scale val: %f, Offset val: %f\n", 
           _config.nantennas(), _config.cb_tscrunch(), _config.cb_fscrunch(), _config.npol(), 
    ...
    */
    ...
    std::size_t ntimestamps = max(1L, SKYWEAVER_CB_PACKET_SIZE/(_config.nchans()/_config.cb_fscrunch())/(_config.nsamples_per_heap()/_config.cb_tscrunch()));
    ntimestamps = max(ntimestamps, SKYWEAVER_CB_NSAMPLES_PER_BLOCK / _config.nsamples_per_heap());
    ...
    std::size_t weights_size = _config.nantennas() * _config.nchans() * _config.nbeams();
