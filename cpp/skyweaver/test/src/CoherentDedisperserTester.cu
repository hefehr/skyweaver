...
void CoherentDedisperserTester::SetUp()
{
    ...
    CoherentDedisperser::createConfig(dedisp_config,  fft_length, max_delay_samps, nchans, npols, nantennas,tsamp, f_low, bridge_bw, dms);
...
TEST_F(CoherentDedisperserTester, testCoherentDedisperser)
{
    ...
    BOOST_LOG_TRIVIAL(info) << "testCoherentDedisperser input h_voltages.size(): " << h_voltages.size();
    ...
    BOOST_LOG_TRIVIAL(info) << "testCoherentDedisperser output d_voltages_out.size(): " << d_voltages_out.size();


    unsigned int freq_idx = 0;
    unsigned int dm_idx = 0;

    coherentDedisperser.dedisperse(d_voltages, d_voltages_out, freq_idx, dm_idx);
...
TEST_F(CoherentDedisperserTester, testCoherentDedisperserWithPythonData)
{
    ...
    //input file name is of format codedisp_input_DM<dm>.dat build the filename here
    std::string inp_filename = "/homes/vkrishnan/dev/beamformer/skyweaver/cpp/skyweaver/test/data/dedispersion/codedisp_input_DM" + std::to_string(int(dm)) + "_1chan.dat";
    std::string out_filename = "/homes/vkrishnan/dev/beamformer/skyweaver/cpp/skyweaver/test/data/dedispersion/codedisp_output_DM" + std::to_string(int(dm)) + "_1chan.dat";
    ...
    codedisp_input.read(reinterpret_cast<char *>(h_voltages.data()), inp_filesize);
    BOOST_LOG_TRIVIAL(debug) << "Input h_voltages.size(): " << h_voltages.size();
    ...
    codedisp_output.read(reinterpret_cast<char *>(h_voltages_check.data()), out_filesize);
    BOOST_LOG_TRIVIAL(debug) << "Python output h_voltages_check.size(): " << h_voltages_check.size();
    ...
    thrust::device_vector<char2> d_voltages_out(block_length_tpa - max_delay_tpa);
    BOOST_LOG_TRIVIAL(info) << "cpp output d_voltages_out.size(): " << d_voltages_out.size();
    ...
    for(int i =0; i < h_voltages_out.size(); i++)
    {
        ASSERT_NEAR(h_voltages_out[i].x, h_voltages_check[i].x, acceptable_error);
        ASSERT_NEAR(h_voltages_out[i].y, h_voltages_check[i].y, acceptable_error);
    }
...
TEST_F(CoherentDedisperserTester, testCoherentDedisperserWithPythonDataAllChans)
{
    ...
    //input file name is of format codedisp_input_DM<dm>.dat build the filename here
    std::string inp_filename = "/homes/vkrishnan/dev/beamformer/skyweaver/cpp/skyweaver/test/data/dedispersion/codedisp_input_DM" + std::to_string(int(dm)) + ".dat";
    std::string out_filename = "/homes/vkrishnan/dev/beamformer/skyweaver/cpp/skyweaver/test/data/dedispersion/codedisp_output_DM" + std::to_string(int(dm)) + ".dat";
    ...
    h_voltages.resize(inp_filesize / sizeof(char2));
    codedisp_input.read(reinterpret_cast<char *>(h_voltages.data()), inp_filesize);
    BOOST_LOG_TRIVIAL(debug) << "codedisp_input h_voltages.size(): " << h_voltages.size();
    ...
    BOOST_LOG_TRIVIAL(debug ) << "cpp output d_voltages_out.size(): " << d_voltages_out.size();

    for (int i = 0; i < nchans; i++)
    {
        // becasuse the vector's length is used in calculations inside the code. It cannot take FTPA

        thrust::device_vector<char2> d_tpa_voltages(block_length_tpa);
        thrust::copy(d_voltages.begin() + i*block_length_tpa, 
                    d_voltages.begin() + (i+1)*block_length_tpa, d_tpa_voltages.begin()); 
        unsigned int freq_idx = i;
        unsigned int dm_idx = 1;
        coherentDedisperser.dedisperse(d_tpa_voltages, d_voltages_out, freq_idx, dm_idx);
...
TEST_F(CoherentDedisperserTester, testCoherentDedisperserChirpMultiply)
{
    ...
    thrust::host_vector<cufftComplex> h_chirp(fft_length, make_cuComplex(1.0, 1.0));
    random_normal_complex(h_chirp,
        fft_length,
        0.0f,
        17.0f);
