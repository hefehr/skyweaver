...
TYPED_TEST(IncoherentDedisperserTester, zero_dm_delays_test)
{
    ...
    ASSERT_EQ(delays.size(), 1 * this->_config.channel_frequencies().size())
...
TYPED_TEST(IncoherentDedisperserTester, ones_test)
{
    ...
    for(auto const& val: output) {
        EXPECT_EQ(val,
                  value_traits<typename Traits::OutputType>::one() *
                      std::sqrt(this->_config.nchans()));
    }
}
...
TYPED_TEST(IncoherentDedisperserTester, ones_test_wtscrunch)
{
    ...
    for(auto const& val: output) {
        EXPECT_EQ(val,
                  static_cast<typename Traits::OutputType>(
                      value_traits<typename Traits::OutputType>::one() *
                      std::sqrt(this->_config.nchans() * tscrunch)));
    }
}
