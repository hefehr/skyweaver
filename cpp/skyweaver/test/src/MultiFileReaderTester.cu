...
MultiFileReaderTester::MultiFileReaderTester(): ::testing::Test()
{
    ...
    for(int i=0; i<10; i++)
    {
        std::string file_name = "data/dada_files/data_chunk_" + std::to_string(i) + ".dada";
...
{    std::string full_file_name = "data/dada_files/data.dada";
    ...
    std::size_t nsamples_multi_file = multi_file_reader->get_total_size() / (nantennas * nchans * npols * nbits * 2 / 8);
    BOOST_LOG_TRIVIAL(debug) << "nsamples from multi_file_reader: " << nsamples_multi_file;

    std::ifstream full_file(full_file_name, std::ios::binary);
    full_file.seekg(0, std::ios::end);
    int file_size = full_file.tellg();
    std::size_t nsamples_full_file = (file_size - pipeline_config.dada_header_size())/ (nantennas * nchans * npols * nbits * 2 / 8);
    BOOST_LOG_TRIVIAL(debug) << "nsamples from full_file_reader: " << nsamples_full_file;
    ...
    thrust::host_vector<char2> multi_file_voltages(char2s_per_sample * nsamples_gulp);
    thrust::host_vector<char2> full_file_voltages(char2s_per_sample * nsamples_gulp);    
    ...
    full_file.read(reinterpret_cast<char*>(thrust::raw_pointer_cast(full_file_voltages.data())), 
                    static_cast<std::streamsize>(nantennas * nchans * npols * nsamples_gulp ));
...
TEST_F(MultiFileReaderTester, testMultiFileReaderEOF)
{
    ...
    multi_file_reader->seekg(multi_file_reader->get_total_size(), std::ios::beg);
    ASSERT_EQ(multi_file_reader->eof(), true);

    //this should give a run time error - test it
    ASSERT_THROW(multi_file_reader->seekg(multi_file_reader->get_total_size() + 1, std::ios::beg), std::runtime_error);
}
...
TEST_F(MultiFileReaderTester, testMultiFileReaderNonExistentFile)
{
    ...
    pipeline_config.input_files(files) ;
    ASSERT_THROW(std::make_unique<MultiFileReader>(pipeline_config), std::runtime_error);
}
