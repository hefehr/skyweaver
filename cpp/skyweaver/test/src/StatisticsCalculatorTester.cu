...
{
    std::size_t fpa_size = _config.nantennas() * _config.npol() * _config.nchans();
        ...
        for(std::size_t t = 0; t < nsamples; ++t) {
            for(std::size_t p = 0; p < _config.npol(); ++p) {
                for(std::size_t a = 0; a < _config.nantennas(); ++a) {
                    std::size_t output_idx = (f * _config.nantennas() * _config.npol()) +
    ...
    for(std::size_t stats_idx = 0; stats_idx < fpa_size; ++stats_idx) {
        ...
        ASSERT_DOUBLE_EQ(gpu_results[stats_idx].skew, stats[stats_idx].skewness());
        ASSERT_DOUBLE_EQ(gpu_results[stats_idx].kurtosis, stats[stats_idx].kurtosis());
    }
}

TEST_F(StatisticsCalculatorTester, test_zero_array)
{
    // Make some input data
    std::size_t nsamples = 1024;
    std::size_t input_size =
        _config.nantennas() * _config.npol() * _config.nchans() * nsamples;
    thrust::device_vector<char2> ftpa_voltages(input_size, {0, 0});
    ...
    stats_h = stats;
    thrust::host_vector<char2> ftpa_voltages_h = ftpa_voltages;
